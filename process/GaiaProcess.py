...
class GaiaProcess(Process):
    ...
    def process(self, reconstruct=False):
        ...
        self.logs = np.load(log_path, allow_pickle=True)
        ...
        for case_id, v in tqdm(demo_traces.items()):
            ...
            if not self.metrics is None:
                demo_metrics[case_id] = {
                    x: [[y[0], "{}_{}_{}".format(y[1], y[2], "+" if y[3] > 0 else "-")] for y in
            ...
            if not self.traces is None:
                demo_traces[case_id] = {
                    x: [[y[0], "{}_{}".format(y[1], y[2])] for y in self.traces[str(case_id)] if
                        x[0] in y[2] or x[0] in y[1]] for x in inner_dict_key}
            ...
            if not self.logs is None:
                demo_logs[case_id] = {
                    x: [[y[0], y[2]] for y in self.logs[case_id] if
    ...
    def build_dataset(self):
        ...
        label_types = ['anomaly_type', 'instance', 'service']
